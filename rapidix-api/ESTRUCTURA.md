...
| Archivo | Qué contiene |
| --- | --- |
| [rutas.controller.ts](src/rutas/rutas.controller.ts) | `GET /admin/rutas` — el tablero: la jornada **y** los pedidos de la pestaña en una sola respuesta, porque todo lo que el repartidor puede pulsar depende de su jornada y son dos datos de la misma pantalla de teléfono. Pestañas: `disponibles` (lo que espera en bodega, de todos), `en-camion` y `entregados` (lo suyo, sin liquidar); trae también las `entregas` de la jornada con sus conteos. Más `POST jornada`, `POST jornada/finalizar`, `POST entregas` (crea la siguiente, nombre opcional), `GET entregas/:id` (sus pedidos y lo disponible para agregarle), `POST pedidos/:id/recolectar` (exige `entregaId`; 409 `EN_OTRA_ENTREGA`), `POST pedidos/:id/quitar-de-entrega` (solo en RECOLECTADO: borra la carga y vuelve a bodega), `POST pedidos/:id/en-ruta`, `POST pedidos/:id/entregar`, `POST pedidos/:id/no-entregar`, `GET corte` (lo que dice el sistema que trae, sin cambiar nada), `POST corte` y `PATCH cortes/:id`. |
