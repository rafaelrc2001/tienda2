...
- `EntregaRuta` — un viaje dentro de la jornada (Entrega 1, 2… por `@@unique([sesionId, numero])`, nombre opcional). Puede haber varias a la vez; el corte sigue siendo uno por jornada. **Un pedido va en una sola**: `Pedido.entregaRutaId` es una columna; se suelta al quitarlo antes de salir o al regresar a bodega en el corte.
...
- `Corte` — el cierre de caja: calculado, declarado y contado. Un `CHECK` exige que los tres datos de la recepción vayan juntos o no vayan. **Quien recibe no puede ser quien cerró**, salvo el administrador (que puede ser el único usuario): esa regla depende del rol, así que la cuida `CortesService.recibir()` y no la base.
...
| Archivo | Qué contiene |
| --- | --- |
| [rutas.controller.ts](src/rutas/rutas.controller.ts) | `GET /admin/rutas` — el tablero: la jornada **y** los pedidos de la pestaña en una sola respuesta, porque todo lo que el repartidor puede pulsar depende de su jornada y son dos datos de la misma pantalla de teléfono. Pestañas: `disponibles` (lo que espera en bodega, de todos), `en-camion` y `entregados` (lo suyo, sin liquidar); trae también las `entregas` de la jornada con sus conteos. Más `POST jornada`, `POST entregas` (crea la siguiente, nombre opcional), `GET entregas/:id` (sus pedidos y lo disponible para agregarle), `POST entregas/:id/finalizar` y `entregas/:id/reanudar` (409 `ENTREGA_FINALIZADA` al mover algo de una entrega finalizada), `POST pedidos/:id/recolectar` (exige `entregaId`; 409 `EN_OTRA_ENTREGA`), `POST pedidos/:id/quitar-de-entrega` (solo en RECOLECTADO: borra la carga y vuelve a bodega), `POST pedidos/:id/en-ruta`, `POST pedidos/:id/entregar`, `POST pedidos/:id/no-entregar`, `GET entregas/:id/corte` (lo que dice el sistema que trae de esa entrega, sin cambiar nada), `POST entregas/:id/corte` (corta solo esa entrega; el de la última viva cierra también la jornada) y `PATCH cortes/:id`. |
...
| [cortes.service.ts](src/rutas/cortes.service.ts) | `cerrar()` — el cierre del día en una transacción: nace el corte con lo calculado y lo declarado **uno al lado del otro** (para que Finanzas vea la diferencia en vez de descubrirla contando), los pedidos entregados quedan liquidados, **se descarga el camión** y la jornada muere atada a su corte. `descargarCamion()` escribe lo devuelto en cada renglón, lo cierra, lo reingresa a bodega y devuelve a la cola los pedidos que no se entregaron: **el único retroceso del eje físico**, que no pasa por `TRANSICIONES` porque no es un paso adelante sino la constatación de que la mercancía volvió. `recibir()` comprueba que quien cuenta no sea quien cerró (salvo el administrador); `abonar()` deja lo que se entregue después en su propia fila, sin reescribir los montos del día. |
