...
describe("direccionInicial", () => {
  it("manda el borrador del pedido sobre el perfil", () => {
    ...
    expect(direccionInicial(borrador, perfil()).calle).toBe(
      "Casa de mi mamá 5",
    );
  });
  ...
  it("si el perfil no dice quién recibe, recibe el usuario", () => {
    ...
    expect(
      direccionInicial(null, perfil({ quienRecibe: "Lupita" })).quienRecibe,
    ).toBe("Lupita");
  });
  ...
  it("un borrador sin domicilio no tapa la dirección guardada en el perfil", () => {
    const d = direccionInicial(
      { ...direccionVacia(), referencias: "Timbre roto" },
      perfil(),
    );
    expect(d).toMatchObject({
      calle: "Reforma 12",
  ...
  it("no mezcla el domicilio del borrador con el del perfil", () => {
    const borrador = { ...direccionVacia(), calle: "Casa de mi mamá 5" };
    const d = direccionInicial(borrador, perfil());
    expect(d.calle).toBe("Casa de mi mamá 5");
    expect(d.colonia).toBe("");
    expect(d.lat).toBeNull();
  });

  it("completa quién recibe y teléfono vacíos del borrador con el perfil", () => {
    const d = direccionInicial(
      { ...valida, quienRecibe: " ", telefono: "" },
      perfil(),
    );
    expect(d.calle).toBe(valida.calle);
    expect(d.quienRecibe).toBe("Rubén Pérez");
    expect(d.telefono).toBe("9931234567");
  });
});
...
describe("validación y resumen", () => {
  ...
  it("estado y referencias son opcionales", () => {
    expect(
      erroresDireccion({ ...valida, estado: "", referencias: "" }),
    ).toEqual({});
  });
  ...
  it("resume quién recibe, calle y ciudad saltando lo vacío", () => {
    ...
    expect(resumenDireccion({ ...valida, quienRecibe: " " })).toBe(
      "Reforma 12 · Villahermosa",
    );
